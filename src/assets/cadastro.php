    ...
    (function () {
        ...
        btnAnali.addEventListener('click', async () => {
            ...
            try {
                ...
                // API v12 retorna: {"anos": {"ano6": {"disciplinas":{...},"notas":[...]}, "ano7":{...}}}
                if (!json.anos || Object.keys(json.anos).length === 0) {
                ...
                // Descobre os anos presentes (chaves "ano6".."ano9") e define o mais recente para o badge
                const anosPresentes = Object.keys(json.anos)
                    .map(k => parseInt(k.replace('ano', '')))
                    .filter(n => n >= 6 && n <= 9)
                    .sort((a, b) => b - a);
                ...
                Object.entries(json.anos).forEach(([chaveAno, dadosAno]) => {
                    // chaveAno = "ano6", "ano7", "ano8" ou "ano9"
                    ...
                    // dadosAno.disciplinas é o objeto {portugues: 8.5, matematica: 9.0, ...}
                    const discs = dadosAno.disciplinas || {};

                    DISCIPLINAS_TABELA.forEach((disc, discIdx) => {
                        // religiao agora tem campo próprio na tabela
                        // filosofia → preenche religiao se religiao não existir
                        let chave = disc;
